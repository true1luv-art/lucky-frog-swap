/**
 * server/farm-smart-contract/lib/memo.ts
 *
 * Memo helpers for EVM chain transactions.
 *
...
export function buildWithdrawMemo(ref: string): string {
  return `hfarm:withdraw:${ref}`;
}

/** Namespaced memo for a marketplace purchase payout (seller credit). */
export function buildMarketplaceMemo(listingId: string): string {
  return `hfarm:marketplace:${listingId}`;
}
