...
function makeState(overrides: Partial<GameState> = {}): GameState {
  return {
    ...
    items: { Egg: new Decimal(5), Milk: new Decimal(3), Wool: new Decimal(2) },
    fields:    {},
    trees:     {},
    stones:    {},
    iron:      {},
...
describe("sellProduce", () => {
  ...
  it("throws when insufficient produce", () => {
    const state = makeState({ items: { Egg: new Decimal(1) } });
  ...
  it("increases balance and deducts inventory for Egg", () => {
    ...
    expect(new Decimal(result.items["Egg"]!).toNumber()).toBe(3);
  });
  ...
  it("increases balance and deducts inventory for Milk", () => {
    ...
    expect(new Decimal(result.items["Milk"]!).toNumber()).toBe(2);
  });
  ...
  it("increases balance and deducts inventory for Wool", () => {
    ...
    expect(new Decimal(result.items["Wool"]!).toNumber()).toBe(1);
  });
