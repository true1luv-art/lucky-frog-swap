...
function makeState(overrides: Partial<GameState> = {}): GameState {
  return {
    ...
    items: { "Potato Seed": new Decimal(5) },
    fields:    {},
    trees:     {},
    stones:    {},
    iron:      {},
...
describe("plant", () => {
  it("plants a crop and deducts seed from inventory", () => {
    ...
    expect(new Decimal(result.items["Potato Seed"]!).toNumber()).toBe(4);
  });
  ...
  it("enforces the selected seed farming level", () => {
    const state = makeState({ items: { "Kale Seed": new Decimal(1) } });
    expect(() =>
