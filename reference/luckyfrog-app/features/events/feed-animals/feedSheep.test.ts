...
function makeState(overrides: Partial<GameState> = {}): GameState {
  return {
    ...
    items: { Sheep: new Decimal(2), Parsnip: new Decimal(5) },
    fields:    {},
    trees:     {},
    stones:    {},
    iron:      {},
...
describe("feedSheep", () => {
  ...
  it("throws when not enough Parsnip", () => {
    const state = makeState({ items: { Sheep: new Decimal(2), Parsnip: new Decimal(0) } });
    expect(() => feedSheep({ state, action: { type: "sheep.feed", index: 0 }, createdAt: NOW })).toThrow("Not enough Parsnip to feed sheep");
  });

  it("feeds an unfed sheep and deducts 2x Parsnip", () => {
    const state  = makeState();
    const result = feedSheep({ state, action: { type: "sheep.feed", index: 0 }, createdAt: NOW });
    expect(result.sheep[0].fedAt).toBe(NOW);
    expect(new Decimal(result.items["Parsnip"]!).toNumber()).toBe(3);
  });
