 ...
 * │ CategoryRail: (Featured)(Resources)(Seeds)…          │
 * │ Content: responsive card grid (only this scrolls)    │
 * │   + detail drawer (right on sm:+, bottom sheet on xs)│
 * │ TickerBar: volume · trades · listings                │
 * └───────────────────────────────────────────────────────┘
 *
 * Purely presentational — listings/topSales/stats come in as props so the
 * live modal and the /test-modals mock shell share one layout.
 */
...
export interface MarketplaceListing {
  ...
  seller:    string;
  featured?: boolean;
  /** Lock state — present when another player is actively buying this listing. */
...
export type BuyPhase = "idle" | "locking" | "signing" | "processing";
...
interface Props {
  ...
  onBuy?: (
    ...
    setPhase: (p: BuyPhase) => void,
  ) => Promise<void> | void;
  /** Called when the player closes/cancels the detail drawer so the lock can be released. */
  onCancelBuy?: (listingId: string) => void;
}
...
function useLockCountdown(lockedUntil: string | null | undefined): string | null {
  const [remaining, setRemaining] = useState<string | null>(null);

  useEffect(() => {
    if (!lockedUntil) { setRemaining(null); return; }
...
function ListingCardItem({
  ...
  const isLocked = !!(listing.lockedBy && listing.lockedUntil);

  return (
    <button
      type="button"
      onClick={onClick}
      className={`text-left transition-all duration-75 cursor-pointer relative ${
        selected ? "brightness-110 -translate-y-0.5" : "hover:brightness-110"
      } ${isLocked ? "opacity-60" : ""}`}
      aria-pressed={selected}
      aria-label={`${listing.name}${isLocked ? " (reserved)" : ""}`}
    >
        ...
        {isLocked && (
          ...
          >
            <div className="bg-black/70 rounded px-1.5 py-0.5 flex items-center gap-1">
              <span className="text-[9px] text-yellow-400 text-shadow">Reserved</span>
            </div>
          </div>
        )}
      </InnerPanel>
...
function DetailDrawer({
  ...
  const buying = buyPhase !== "idle";

  // Live countdown for locked listings.
  const countdown = useLockCountdown(listing.lockedUntil);
  const isLockedByOther = !!(listing.lockedBy && countdown);

  // Reset phase when listing changes.
  const prevId = useRef(listing.id);
  useEffect(() => {
    if (prevId.current !== listing.id) {
  ...
  const buyLabel =
    buyPhase === "locking"    ? "Reserving…"
    : buyPhase === "signing"    ? "Sign in wallet…"
    : buyPhase === "processing" ? "Processing…"
    : isLockedByOther           ? `Reserved (${countdown})`
    : "Buy";
    ...
    <OuterPanel className="flex flex-col gap-1 max-h-full sm:h-full overflow-hidden">
      ...
      <InnerPanel className="flex-1 min-h-0 overflow-y-auto flex flex-col items-center gap-2 p-3">
        ...
        <div
          ...
          style={listing.rarity ? {
            background: `radial-gradient(circle, ${RARITY_STRIPE[listing.rarity]}33 0%, transparent 70%)`,
          } : undefined}
        >
          <img
            src={listing.image || "/placeholder.svg"}
        ...
        {listing.rarity && (
          <p className={`text-[10px] capitalize ${RARITY_TEXT[listing.rarity]}`}>
            {listing.rarity}
          </p>
        )}
        ...
        <div className="flex items-center gap-1.5">
          <img src={tokenIcon || "/placeholder.svg"} alt="LFRG" className="w-4 h-4 pixelated" />
          <span className="text-white text-shadow text-sm font-semibold">
            {listing.price.toLocaleString()}
          </span>
          {listing.usd && (
            <span className="text-white/50 text-[10px]">{listing.usd}</span>
          )}
        </div>

        <p className="text-white/50 text-[9px]">Seller: {listing.seller}</p>

        {/* Lock notice */}
        {isLockedByOther && (
          <div className="w-full rounded border border-yellow-700/60 bg-yellow-900/30 px-2 py-1.5 flex flex-col items-center gap-0.5">
            <p className="text-yellow-400 text-[9px] text-shadow text-center leading-snug">
              Reserved by another player
            </p>
            <p className="text-yellow-400/80 text-[9px] font-semibold">
        ...
        {/* Buy error */}
        {buyError && (
          <p className="text-red-400 text-shadow text-[9px] text-center text-balance px-1">
            {buyError}
          </p>
        )}
      </InnerPanel>
        ...
        info={
          <span className="truncate">
            {buying
              ...
              : lfrgBalance !== undefined
                ? `Balance: ${lfrgBalance.toLocaleString()} LFRG`
                : "Pays on-chain in LFRG"}
          </span>
        }
      >
        <Button
          className="text-xs px-3 w-auto"
          disabled={buying || isLockedByOther}
...
export const MarketplaceScreen: React.FC<Props> = ({
  ...
  // Sync selected listing with live data so lock badge stays current.
  const selectedLive = useMemo(() => {
    if (!selected) return null;
    return listings.find((l) => l.id === selected.id) ?? selected;
  }, [selected, listings]);
  ...
  const handleCancelBuy = () => {
    if (selectedLive) onCancelBuy?.(selectedLive.id);
  };

  const handleBuy = async () => {
    if (!selectedLive || !onBuy) throw new Error("Nothing to buy.");
    // The buy phase is managed inside DetailDrawer; we just need a setter.
    // onBuy already holds the phase setter injected by MarketplaceModal.
    await onBuy(selectedLive, () => {});
    setSelected(null);
  };
  ...
  return (
    ...
    >
      ...
      <InnerPanel className="flex flex-wrap items-center gap-2 px-2 py-1.5 shrink-0">
        ...
        />
        <label className="sr-only" htmlFor="marketplace-sort">Sort listings</label>
        <select
          id="marketplace-sort"
          value={sort}
        ...
        <label className="sr-only" htmlFor="marketplace-rarity">Filter by rarity</label>
        <select
