...
export interface ResourceNode extends SpatialNode {
  ...
  hitCount?: number;
  depletedAt?: number;
  /** Timestamp (ms) of the most recent hit — used for per-hit cooldown gating. */
  lastHitAt?: number;
  /** Phaser TimerEvent that resets hitCount/overlay after 10 s of no interaction. */
  idleResetTimer?: Phaser.Time.TimerEvent;
  /** Phaser TimerEvent that restores the node after the recovery period. */
  replenishTimer?: Phaser.Time.TimerEvent;
  sprite?: Phaser.GameObjects.Image | Phaser.GameObjects.Rectangle;
  animSprite?: Phaser.GameObjects.Sprite;
  progressOverlay?: Phaser.GameObjects.Image;
  /** Static physics body used for player collision — removed when node is depleted, restored on respawn. */
...
export interface PlotNode extends SpatialNode {
  ...
  /** Shown above crop when planted but not yet watered */
  waterIcon?: Phaser.GameObjects.Image | null;
}
